...
func main() {
	...
	//generic flags
	service := flag.String("service", "gitlab", "Git Hosted Service Name (github/gitlab)")
	githostUrl := flag.String("githost.url", "https://git-scm.co/", "DNS of the custom Git host")
	...
	if err != nil {
		...
		for _, repo := range repos {
			...
			go func(repo *Repository) {
				...
				if err != nil {
					log.Printf("Error backing up %d:%s\n", repo.name, stdoutStderr)
				}
				<-tokens
			}(repo)
		}
